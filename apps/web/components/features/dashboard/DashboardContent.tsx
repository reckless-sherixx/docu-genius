...
export default function DashboardContent() {
  ...
  useEffect(() => {
    ...
    const unsubscribeMemberRemove = onMemberRemoved((data) => {
      setTeamMembers((prev) => prev.filter((m) => m.user_id !== data.memberId));
    });

    const unsubscribeRoleUpdate = onMemberRoleUpdated((data) => {
      setTeamMembers((prev) =>
        prev.map((m) =>
          m.user_id === data.memberId ? { ...m, role: data.newRole } : m,
        ),
      );
    });
  ...
  const lastUpdateTime = latestDocument
    ? getTimeSince(latestDocument.created_at)
    : "No updates";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-lg border border-gray-200">
            ...
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-purple-600 text-xs hidden sm:inline">
                Active
              </span>
            </div>
                ...
                {loadingDocs ? (
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-xs mt-1">
                      Create a template and generate your first document
                    </p>
                  </div>
                ) : (
